...
### 3.5 global_rebalance_insights (전수 인사이트 정규화)
- **목적:** 마스터 데이터 변경 시 과거 모든 인사이트의 JSON 스냅샷을 전수 조사하여 최신 정보로 강제 동기화.
- **핵심 로직:** 티커가 아닌 **'이름(name_ko)'** 및 **'별칭(aliases)'** 매칭을 통해 불일치 데이터를 찾아내고, 마스터 테이블의 최신 티커와 검증 상태로 일괄 업데이트함.
