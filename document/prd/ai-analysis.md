...
> **[기술적 개선 사항] ID 오타 방지 전략 (2026-03-11)**
> Gemini AI가 긴 UUID(36자)를 결과 JSON에 그대로 복사할 때 글자 하나를 빼먹는 오타(Transcribing Error)가 발생하여 DB 저장 시 `invalid input syntax for type uuid` 오류가 발생하는 문제가 확인되었습니다. 이를 해결하기 위해 프롬프트에는 단순한 **숫자 인덱스(number)**를 사용하고, 코드 레벨에서 다시 UUID로 복원하는 매핑 로직을 도입하여 시스템의 견고함을 높였습니다.
...
| 필드명 | 타입 | 설명 |
| :--- | :--- | :--- |
| **feed_id** | number | 원본 피드 리스트의 인덱스 (0, 1, 2...) |
